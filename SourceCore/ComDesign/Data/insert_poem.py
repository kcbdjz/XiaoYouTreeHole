...
#保存处理后的csv文件
df.to_csv('./poem_cleaned.csv', index=False)

# 读取csv文件
data = pd.read_csv('./poem_cleaned.csv',encoding='utf8')
#data = pd.read_csv('./poem.csv',encoding='utf8')
# 连接到MySQL数据库
...
for index, row in data.iterrows():
    id = id+1
    sql = f"INSERT INTO poem (text) VALUES ('{row['text']}')"
    cursor.execute(sql)
